...
export default function JoinCard() {
  ...
  return (
    <Card className="col-md-6 mx-auto p-0 border-0 card-shadow rounded-4">
      <Title>{publicRoom?.data.name}</Title>
      <Card.Body className="pt-4 px-4">
        <Row className="align-items-center">
          <Col className="col-xxl-8">
            <span className="text-muted fs-5">{t('room.meeting.meeting_invitation')}</span>
            ...
            { (recordValue !== 'false') && recordings?.data?.length > 0 && (
              <ButtonLink
                ...
                className="mt-3 mb-0 cursor-pointer btn-sm"
          ...
          <Col>
            <Stack direction="vertical" gap={2} className="text-center">
              <Avatar className="d-block mx-auto" avatar={publicRoom?.data.owner_avatar} size="medium" />
              <h5 className="mb-0">{publicRoom?.data.owner_name}</h5>
            </Stack>
          </Col>
        </Row>
      </Card.Body>
      <Card.Footer className="px-4 pb-4 bg-white border-0">
        <Row>
          {(roomStatusAPI.isSuccess && !roomStatusAPI.data.status) ? WaitingPage : (
            <Form methods={methods} onSubmit={handleJoin}>
              ...
              <Button
                variant="brand"
                className="mt-2 d-block w-100 py-2"
                type="submit"
